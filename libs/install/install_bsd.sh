...
install_bsd() {
    cd $pwd && chmod +x helm && ./helm install clickhouse-cluster tools/clickhouse -f config/$env/middleware/clickhouse.yaml -n$bsd_ns --create-namespace
    cd $pwd && chmod +x helm && ./helm install redis tools/redis/redis -n$bsd_ns
    cd $pwd && chmod +x helm && ./helm install bsd bsd -f config/$env/bsd/$party_id.yaml -n$bsd_ns
